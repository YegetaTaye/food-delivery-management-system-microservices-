...
export const startEventConsumer = async (): Promise<void> => {
  try {
    ...
    logger.info(`   - Listening for: ${ORDER_CREATED_KEY}, ${ORDER_STATUS_UPDATED_KEY}, ${PAYMENT_COMPLETED_KEY}`);
...
const handleOrderCreated = async (event: OrderCreatedEvent): Promise<void> => {
  const message = `Your order #${event.orderId} has been created successfully! Total: $${event.total.toFixed(2)}`;
